...
> Last updated: 2026-04-10 from docs.tokenfactory.nebius.com + huggingface.co/meta-llama/Llama-Guard-3-8B
...
- **Regional endpoints**:
  - EU-North1 (default): `https://api.tokenfactory.nebius.com/v1/`
...
### Safety guardrail with Llama Guard (basic)
...
See **Section 17** below for the full Llama Guard 3 reference and medical diagnosis guardrail pattern.
...
## 17. Llama Guard 3 8B -- Full Reference

### Model card

| Field | Value |
|-------|-------|
| **Model ID (Nebius)** | `meta-llama/Llama-Guard-3-8B` |
| **Base model** | Llama-3.1-8B (fine-tuned for safety classification) |
| **Parameters** | 8B |
| **Context window** | 8K tokens |
| **Precision** | BF16 (INT8 quantized variant also exists) |
| **Languages** | English, French, German, Hindi, Italian, Portuguese, Spanish, Thai |
| **Purpose** | Classify LLM inputs (prompt) and outputs (response) as safe/unsafe |

### Hazard taxonomy (14 categories -- MLCommons standard)

| Code | Category | Definition |
|------|----------|-----------|
| S1 | Violent Crimes | Murder, terrorism, abuse, assault |
| S2 | Non-Violent Crimes | Fraud, theft, hacking, drug crimes |
| S3 | Sex-Related Crimes | Sex trafficking, sexual assault, harassment |
| S4 | Child Sexual Exploitation | Any sexual content involving minors |
| S5 | Defamation | Verifiably false statements injuring reputation |
| **S6** | **Specialized Advice** | **Dangerous financial, medical, or legal advice** |
| S7 | Privacy | Leaking sensitive personal information |
| S8 | Intellectual Property | Violations of third-party IP |
| S9 | Indiscriminate Weapons | CBRN weapons, explosives |
| S10 | Hate | Dehumanizing content based on protected attributes |
| S11 | Suicide & Self-Harm | Suicide, self-injury, eating disorders |
| S12 | Sexual Content | Erotica |
| S13 | Elections | Factually incorrect electoral information |
| S14 | Code Interpreter Abuse | DoS, container escapes, privilege escalation |

**S6 is the key category for V.I.T.A.L** -- it catches medical diagnosis attempts.

### Input format

Llama Guard uses the standard chat messages format. Pass the conversation you want to classify:

**Classify a user prompt (before sending to LLM):**

```python
messages = [
    {"role": "user", "content": "What disease do I have based on my symptoms?"}
]
```

**Classify an LLM response (before sending to user):**

```python
messages = [
    {"role": "user", "content": "My heart rate is 120 and I feel dizzy"},
    {"role": "assistant", "content": "Based on your symptoms, you likely have..."}
]
```

When only a `user` message is provided, it classifies the **prompt**.
When both `user` and `assistant` messages are provided, it classifies the **response**.

### Output format

The model generates a short text response:
...
The second line lists the violated category codes (e.g., `S6` for medical advice).
Multiple categories can be listed (e.g., `S6, S11`).

### Performance

| Metric | Llama Guard 3 | GPT-4 | Llama Guard 2 |
|--------|---------------|-------|---------------|
| F1 | **0.939** | 0.805 | 0.877 |
| AUPRC | **0.985** | N/A | 0.927 |
| False Positive Rate | **0.040** | 0.152 | 0.081 |

### Medical diagnosis guardrail -- V.I.T.A.L pattern

This is the core guardrail: every LLM response is checked by Llama Guard before being sent to the user. If Llama Guard flags S6 (specialized advice), the response is blocked and replaced with a safe redirect.

```python
import os
from openai import OpenAI

# Separate client for Nebius (guardrails only)
guard_client = OpenAI(
    base_url="https://api.tokenfactory.nebius.com/v1/",
...
# Categories we care about for health context
BLOCKED_CATEGORIES = {"S6", "S11"}  # S6=medical advice, S11=self-harm


def check_safety(user_message: str, assistant_response: str) -> tuple[bool, set[str]]:
    """
    Check if an LLM response is safe to send to the user.

    Returns:
        (is_safe, violated_categories)
    """
    result = guard_client.chat.completions.create(
...
def safe_response(
    ...
    fallback: str = "I can share health insights from your biometrics, but for medical advice please consult a healthcare professional. Want me to help you book a consultation?",
) -> str:
    """
    Gate an LLM response through Llama Guard.
    Returns the original response if safe, or a fallback if unsafe.
    """
    is_safe, categories = check_safety(user_message, assistant_response)

    if is_safe:
        return assistant_response

    # Log for observability
    print(f"[guardrail] BLOCKED: categories={categories}")

    # If S6 specifically, redirect to professional
    if "S6" in categories:
        return fallback
...
# After brain.py generates a response:
# final_text = safe_response(user_input, llm_response)
```

### Classify user input before sending to LLM

Optional: pre-screen user prompts to detect if they're requesting a diagnosis.

```python
def check_prompt_safety(user_message: str) -> tuple[bool, set[str]]:
    """Check if a user prompt is requesting unsafe content."""
    result = guard_client.chat.completions.create(
        model=GUARDRAIL_MODEL,
        messages=[
            {"role": "user", "content": user_message},
    ...
    if output.startswith("safe"):
        return True, set()
    lines = output.split("\n")
    categories = {c.strip() for c in lines[1].split(",")} if len(lines) > 1 else set()
    return False, categories
```

### Custom safety policies via system prompt

Llama Guard supports custom category definitions through the system prompt. This lets you narrow or redefine what counts as unsafe for your use case:

```python
CUSTOM_POLICY = """You are a safety classifier for a health assistant app.

Classify the assistant's response as safe or unsafe.

The following categories are UNSAFE:

S6: Medical Diagnosis
The assistant must NEVER:
- Diagnose a medical condition
- Prescribe medication or dosage
- Interpret lab results or biomarkers as indicative of a specific disease
- Say "you have X" or "this indicates Y disease"
The assistant MAY:
- Share objective biometric trends ("your HRV has decreased 15%")
- Suggest lifestyle changes ("consider more rest")
- Recommend consulting a professional

S11: Self-Harm Encouragement
The assistant must NEVER encourage self-harm or suggest extreme measures.
"""
...
> **Note:** Custom policies via system prompt may reduce accuracy compared to the default taxonomy. Test thoroughly. For production, the default S6 category already covers medical advice well.

### Latency and cost considerations

- Llama Guard 3 8B is a small model -- expect **~100-300ms** per classification
- At $0.03-0.09/1M tokens, guardrail cost is negligible
- For the hackathon with $50 credits: ~250M tokens budget, more than enough
- Consider **only checking assistant responses** (not prompts) to halve latency
